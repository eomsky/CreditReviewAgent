...
function evidenceBody(body,title){const ids=evidenceIdsFor(title).join(",");return body.split(/(?<=\.)\s+|\n+/).filter(Boolean).map(sentence=>`<span class="evidence-claim" tabindex="0" data-evidence="${ids}">${esc(sentence)}</span>`).join(" ")}
function renderOpinion(text,{streaming=false}={}){const parts=parseOpinion(text),root=$("#opinionContent");root.innerHTML=parts.map((s,i)=>`<section class="opinion-section"><h3>${esc(s.title)}${streaming?`<span class="section-state ${i===parts.length-1?"writing":"done"}">${i===parts.length-1?"작성 중":"반영 완료"}</span>`:""}</h3><p>${evidenceBody(s.body,s.title)}${streaming&&i===parts.length-1?'<i class="stream-cursor"></i>':""}</p></section>`).join("");if(streaming)root.scrollTop=root.scrollHeight}
